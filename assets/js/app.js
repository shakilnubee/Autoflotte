...
FP.aenAnnuelFlotte = (vehicules) => (vehicules || []).reduce((s, v) => {
  const ch = v && v.chauffeur && String(v.chauffeur).trim();
  const excl = ['', '—', 'x', 'X', 'Siège', 'Dépôt', 'Navette', 'VENDU', 'Fenwick'];
  if (!ch || excl.includes(String(v.chauffeur).trim())) return s;
  const d = FP.aenDetail(v);
  return s + (d && d.applicable ? d.netAnnuel : 0);
}, 0);
...
// ⚠️ HELPER CANONIQUE — PRÉNOM normalisé d'un conducteur À PARTIR DE SA CLÉ de fiche.
// ❗ NE JAMAIS déduire le prénom en découpant la clé (`key.split(/[-\s]/)[0]`) : une clé peut être un
// PRÉNOM simple (« charles »), un PRÉNOM-NOM (« charles-lennon »), une forme SANS séparateur
// (« charleslennon ») ou un id — le découpage donne alors n'importe quoi et casse le rattachement
// (bug réel : péages « Charles » jamais reliés au congé de la fiche « Charles LENNON »).
...
FP.SCAN_PROMPT = [
  ...
  "vin : le numero de serie / VIN du vehicule (17 caracteres alphanumeriques, sans I O Q), s il figure sur le document (frequent sur une facture d achat ou une carte grise) ; sinon null.",
  ...
  "- numeroTelepaiement : le numero de telepaiement pour payer en ligne. Sur un avis/notice ANTAI il apparait SOUVENT DEUX FOIS et A L'IDENTIQUE : (1) dans le bloc 'Paiement par carte bancaire avec le n° de telepaiement suivant' (case a case), et (2) sur le coupon 'CARTE DE PAIEMENT' en bas, sous 'N° de Telepaiement'. LIS LES DEUX et VERIFIE qu'ils sont IDENTIQUES, chiffre par chiffre : s'ils different, RELIS attentivement (tu as sans doute lache un chiffre). C'est en general 14 chiffres, presentes en groupes 4-4-4-2 (ex '3336 6005 2814 91'). COMPTE les chiffres et n'en OUBLIE AUCUN (erreur frequente : sauter un chiffre au milieu). Donne UNIQUEMENT les chiffres, sans espaces. Ne le CONFONDS PAS avec le numero d'avis de contravention (en haut a gauche, ~10 chiffres) ni avec la longue reference code-barres du bas.",
  "- cle : la 'Cle' (de telepaiement) associee, EXACTEMENT 2 chiffres, juste a cote (case 'Cle') du numero de telepaiement, presente elle aussi aux DEUX endroits (bloc paiement + coupon) : verifie qu'elle est identique aux deux.",
...
FP.edl = {
  ...
  open(veh, opts) {
    ...
    ov.innerHTML = `<div style="background:#fff;border-radius:16px;max-width:780px;width:100%;max-height:92vh;display:flex;flex-direction:column;overflow:hidden;box-shadow:0 24px 60px -20px rgba(0,0,0,.5)">
      ...
      <div style="padding:16px 18px;overflow:auto;flex:1">
        <div style="display:flex;gap:14px;align-items:center;flex-wrap:wrap;margin-bottom:12px;padding:8px 10px;background:#f8fafc;border-radius:10px;font-size:13px">
          ...
          <label style="display:inline-flex;gap:6px;align-items:center;cursor:pointer"><input type="radio" name="edl-sens" value="restitution" ${sens === 'restitution' ? 'checked' : ''}> 📤 Restitution (sortie)</label>
        </div>
    ...
    const renderThumbs = () => {
      if (!thumbsBox) return;
      thumbsBox.innerHTML = edlPhotos.map((p, i) => `<div style="position:relative"><img src="${p}" style="width:74px;height:74px;object-fit:cover;border-radius:8px;border:1px solid #e2e8f0"><button type="button" data-edl-prm="${i}" title="Retirer" style="position:absolute;top:-6px;right:-6px;background:#ef4444;color:#fff;border:none;border-radius:9999px;width:20px;height:20px;line-height:1;font-size:12px;cursor:pointer;box-shadow:0 2px 6px rgba(0,0,0,.3)">✕</button></div>`).join('');
    ...
    const collect = () => {
      ...
      const sr = ov.querySelector('input[name="edl-sens"]:checked');
      return {
...
FP.vehModal = (function () {
  ...
  function open(query) { // query = 'immat=<enc>' ou 'veh=<enc>'
    ...
    if (!bd) {
      ...
      bd.style.cssText = 'position:fixed;inset:0;z-index:10000;display:none;align-items:center;justify-content:center;padding:0;background:rgba(11,18,32,.6);opacity:0;transition:opacity .2s ease';
      bd.innerHTML = '<div class="fp-vm-box" style="width:100vw;max-width:100vw;height:100vh;height:100dvh;background:var(--fp-surface,#fff);border-radius:0;overflow:hidden;position:relative;box-shadow:none;transform:scale(.99);transition:transform .22s cubic-bezier(.22,.61,.36,1)"><iframe title="Fiche véhicule" style="display:block;width:100%;height:100%;border:0"></iframe></div>';
      document.body.appendChild(bd);
