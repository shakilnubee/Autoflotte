...
0ter. ⚠️ **PERF / « flash » des chiffres & latence — LEÇONS (ne pas refaire les erreurs)** :
  ...
  - 🔒 **RGPD — `data.js` NE DOIT JAMAIS contenir de données personnelles** (le repo est **PUBLIC**) : à chaque régénération, **STRIPPER** de `conducteurs` les champs `email`, `adresse`, `dateNaissance`, `tel` (et tout autre PII : n° permis, n° sécu…) et de `vehicules` le `vin`. Ces données sensibles ne vivent QUE dans Supabase (chargées après login, protégées par RLS). `data.js` = juste un instantané pour l'affichage instantané hors-ligne (immat, marque, km, montants…), pas un annuaire. Vérif avant commit : `grep -c '"email":\|"adresse":\|"dateNaissance":\|"tel":\|"vin":' assets/js/data.js` doit renvoyer **0**.
